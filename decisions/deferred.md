...
| Recognising a session carried on in another, and showing the newer one's state | A dismissal hides a question that was still waiting, or stale questions keep coming back after the button | A wrong match hides a real question; it needs testing against real sessions first ([ADR-ST-006](ADR-ST-006-a-question-can-be-dismissed.md)) |
| Pruning `dismissed.json` | The file passes 1 MB, or reading it shows in the poll time | A line per dismissal; years of use before it matters ([ADR-ST-006](ADR-ST-006-a-question-can-be-dismissed.md)) |
