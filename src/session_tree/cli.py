...
def is_running(port: int) -> bool:
    ...
    try:
        with urllib.request.urlopen(url(port) + "api/ping", timeout=2) as response:  # noqa: S310
...
def start(port: int) -> int:
    ...
    if is_running(port):
        sys.stdout.write(f"already running on {url(port)}\n")
        return 0
    log = (run_dir() / "server.log").open("ab")
    env = dict(os.environ, SESSION_TREE_PORT=str(port))
    ...
    while time.monotonic() < deadline:
        if is_running(port):
            sys.stdout.write(f"started on {url(port)}\n")
            return 0
        time.sleep(0.25)
...
def stop(port: int) -> int:
    ...
    sys.stdout.write(f"stopped {url(port)}\n")
    return 0


def open_window(port: int) -> int:
    ...
    for name in BROWSERS:
        ...
        if not binary:
            continue
        args = [binary, "--new-window", url(port)]
        ...
        sys.stdout.write(f"opened {url(port)} in {name}\n")
        return 0
    sys.stdout.write(f"no browser found; open {url(port)} yourself\n")
    return 0
...
def status(port: int) -> int:
    ...
    sys.stdout.write(f"running on {url(port)}\n")
