...
**Result: 10 of 10 rules turned off the test that names them.** None survived.

| Rule turned off | Tests that went red | The test that names it | Did that one go red? |
|---|---|---|---|
| an abandoned task is kept not removed | 6 | `test_an_abandoned_task_is_kept_not_removed` | yes |
| a started node waiting on open work is blocked | 5 | `test_a_started_node_waiting_on_open_work_is_blocked` | yes |
| a silent started node goes stalled | 5 | `test_a_silent_started_node_goes_stalled` | yes |
| independent goals are separate graphs | 6 | `test_independent_goals_are_separate_graphs` | yes |
| harness noise is not mistaken for a prompt | 5 | `test_harness_noise_is_not_mistaken_for_a_prompt` | yes |
| a half written line is left for next time | 5 | `test_a_half_written_line_is_left_for_next_time` | yes |
| a truncated transcript is reread from the top | 5 | `test_a_truncated_transcript_is_reread_from_the_top` | yes |
| an answered question box is no longer waiting | 5 | `test_an_answered_question_box_is_no_longer_waiting` | yes |
| a finished node no longer asks | 5 | `test_a_finished_node_no_longer_asks` | yes |
| a needs input line counts only when the session is idle | 5 | `test_a_needs_input_line_counts_only_when_the_session_is_idle` | yes |
