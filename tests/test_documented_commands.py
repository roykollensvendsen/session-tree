...
from session_tree.cli import COMMANDS
...
LOOKS_LIKE_A_COMMAND = re.compile(
    r"^\s*\$?\s*(session-tree|uv |git |pytest|python|committed|ruff|mypy)\b", re.MULTILINE
)
...
@pytest.mark.skipif(shutil.which("session-tree") is None, reason="the command is not installed")
...
def test_every_verb_the_command_has_appears_in_a_worked_example():
    ...
    """
    # A verb whose whole job is a side effect -- starting a daemon, opening a
    # browser -- cannot be run here, so a block marked `not run` with its reason
    # counts. What must not happen is a verb appearing in no block at all.
    shown = " ".join(command for _, command, _ in CASES)
    for page in pages():
        shown += " " + page.read_text()
    missing = [v for v in COMMANDS if f"session-tree {v}" not in shown]
    assert not missing, f"no example anywhere uses: {', '.join(missing)}"
